...
const API = "https://api.anthropic.com/v1/messages", MODEL = "claude-sonnet-4-20250514";
...
const STAR_ACTIONS = { ADD_PERSON:{label:"Added a person",stars:15,icon:"👤"}, ADD_OCCASION:{label:"Added an occasion",stars:10,icon:"🗓️"}, COMPLETE_QUIZ:{label:"Completed quiz",stars:30,icon:"🧠"}, BUY_GIFT:{label:"Purchased a gift",stars:20,icon:"🎁"}, RATE_GIFT:{label:"Rated a gift",stars:25,icon:"⭐"}, WRITE_NOTE:{label:"Wrote a note",stars:20,icon:"✍️"}, ENABLE_NOTIF:{label:"Enabled notifications",stars:2,icon:"🔔"}, REFERRAL:{label:"Referred a friend",stars:80,icon:"🤝"} };
...
function getRetailers(name="", emoji="") {
  ...
  const R = {
    flights: [{name:"Skyscanner",emoji:"✈️",color:"#0770E3",bg:"#0770E322",url:`https://www.skyscanner.net/transport/flights/?query=${q}`,badge:"Best prices"},{name:"Google Flights",emoji:"🔍",color:"#4285F4",bg:"#4285F422",url:`https://www.google.com/travel/flights?q=${q}`,badge:"Compare all"},{name:"Kayak",emoji:"🧳",color:"#FF690F",bg:"#FF690F22",url:`https://www.kayak.com/flights?q=${q}`,badge:"Price alerts"}],
    hotels: [{name:"Booking.com",emoji:"🏨",color:"#003580",bg:"#00358022",url:`https://www.booking.com/searchresults.html?ss=${q}`,badge:"Free cancellation"},{name:"Airbnb",emoji:"🏡",color:"#FF5A5F",bg:"#FF5A5F22",url:`https://www.airbnb.com/s/${q}/homes`,badge:"Unique stays"}],
    experience: [{name:"Airbnb Experiences",emoji:"🌟",color:"#FF5A5F",bg:"#FF5A5F22",url:`https://www.airbnb.com/s/experiences?q=${q}`,badge:"Local hosts"},{name:"Viator",emoji:"🗺️",color:"#14B8A6",bg:"#14B8A622",url:`https://www.viator.com/searchResults/all?text=${q}`,badge:"Book & go"},{name:"GetYourGuide",emoji:"🎯",color:"#FF5000",bg:"#FF500022",url:`https://www.getyourguide.com/s/?q=${q}&partner_id=YHVA20C`,badge:"Instant confirm"}],
    events: [{name:"Fever",emoji:"🔥",color:"#EF4444",bg:"#EF444422",url:`https://feverup.com/en/search?q=${q}`,badge:"Top events"},{name:"Ticketmaster",emoji:"🎫",color:"#026CDF",bg:"#026CDF22",url:`https://www.ticketmaster.com/search?q=${q}`,badge:"Official"}],
    spa: [{name:"Treatwell",emoji:"💆",color:"#7C3AED",bg:"#7C3AED22",url:`https://www.treatwell.com/search/?q=${q}`,badge:"Instant booking"},{name:"Spabreaks",emoji:"🛁",color:"#E879A0",bg:"#E879A022",url:`https://www.spabreaks.com/search?q=${q}`,badge:"Packages"}],
    dining: [{name:"OpenTable",emoji:"🍽️",color:"#DA3743",bg:"#DA374322",url:`https://www.opentable.com/s?q=${q}`,badge:"Reserve now"},{name:"Viator",emoji:"🗺️",color:"#14B8A6",bg:"#14B8A622",url:`https://www.viator.com/searchResults/all?text=${q}+food`,badge:"Food tours"}],
    adventure: [{name:"GetYourGuide",emoji:"🎯",color:"#FF5000",bg:"#FF500022",url:`https://www.getyourguide.com/s/?q=${q}&partner_id=YHVA20C`,badge:"Adventures"},{name:"Viator",emoji:"🗺️",color:"#14B8A6",bg:"#14B8A622",url:`https://www.viator.com/searchResults/all?text=${q}`,badge:"Guided"},{name:"Airbnb Experiences",emoji:"🌟",color:"#FF5A5F",bg:"#FF5A5F22",url:`https://www.airbnb.com/s/experiences?q=${q}`,badge:"Local guides"}],
    product: [{name:"Amazon",emoji:"📦",color:"#FF9900",bg:"#FF990022",url:`https://www.amazon.com/s?k=${q}&tag=giftmate0d-20`,badge:"Fast delivery"},{name:"Etsy",emoji:"🎨",color:"#F56400",bg:"#F5640022",url:`https://www.etsy.com/search?q=${q}`,badge:"Handmade"},{name:"Not on the High Street",emoji:"✨",color:"#C8A96E",bg:"#C8A96E22",url:`https://www.notonthehighstreet.com/search?search_term=${q}`,badge:"Personalised"},{name:"UncommonGoods",emoji:"🦋",color:"#5B8C5A",bg:"#5B8C5A22",url:`https://www.uncommongoods.com/search?q=${q}`,badge:"Unique finds"}],
  };
...
function BuyModal({ modal, onOpen, onMarkBought, onClose }) {
  ...
  const { gift } = modal, { retailers, label } = getRetailers(gift.name, gift.emoji||"");
  return (
    <div style={{position:"fixed",inset:0,background:"#00000090",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:P.card,border:`1px solid ${P.border}`,borderRadius:20,padding:28,width:"100%",maxWidth:460,boxShadow:"0 20px 60px #00000066"}}>
        ...
        <div style={{background:`${P.gold}18`,border:`1px solid ${P.gold}33`,borderRadius:10,padding:"9px 13px",marginBottom:12,fontSize:12,color:P.goldL,display:"flex",gap:8,alignItems:"center"}}><span>⭐</span><span>Earn <strong>+20 Stars</strong> when you mark as bought!</span></div>
        <div style={{background:P.indigo+"18",border:`1px solid ${P.indigo}33`,borderRadius:10,padding:"9px 13px",marginBottom:12,fontSize:12,color:P.muted,display:"flex",gap:8}}><span>💡</span><span>Browse retailers — then mark as bought when done.</span></div>
        ...
        <button onClick={()=>onMarkBought(gift)} style={{width:"100%",background:`linear-gradient(135deg,${P.green},#059669)`,color:"#fff",border:"none",borderRadius:10,padding:"11px 0",fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:8}}>✓ I bought it! (+20 Stars)</button>
...
// ── Stars Tab ─────────────────────────────────────────────────────────────────
function StarsTab({ stars, history, onRedeem }) {
...
// ── Main App ──────────────────────────────────────────────────────────────────
export default function Giftmate() {
  ...
  const fetchGifts = async (occ,person) => {
    ...
    catch { setGifts([{id:0,name:"Personalised Photo Book",emoji:"📚",price:45,reason:"A beautiful custom memory book.",category:"product"},{id:1,name:"Artisan Gift Hamper",emoji:"🧺",price:65,reason:"Hand-curated premium treats.",category:"product"},{id:2,name:"Experience Voucher",emoji:"🎟️",price:79,reason:"An unforgettable experience.",category:"experience"}]); }
    setLoadingGifts(false);
  };
  const handleBuy = (gift,fromChat=false) => setBuyModal({gift,person:fromChat?null:selPerson,occasion:fromChat?null:selOcc});
  const openRetailer = (gift,retailer) => { window.open(retailer.url,"_blank"); showToast(`🔗 Opened ${retailer.name}!`); };
  const markBought = gift => { if(!purchased.some(p=>p.name===gift.name)){setPurchased(p=>[...p,{...gift}]); earnStars("BUY_GIFT"); showToast("🎉 Marked as bought! +20 Stars!");} setBuyModal(null); };
  ...
  const redeemReward = reward => { setStars(s=>s-reward.cost); showToast(`🎁 "${reward.title}" redeemed! Check your email.`); };
  const sendChat = async () => {
    ...
    const sys = "You are Giftmate, a warm AI gift concierge. Always suggest a mix of physical gifts AND experiences. If outdoor interests mentioned, suggest outdoor experiences and gear. Use bullet points (-) and **bold** for gift names. Include price ranges ($50-80). Be specific and warm.";
        ...
        {tab==="chat" && (
          <div className="fi" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 150px)"}}>
            ...
            <div style={{display:"flex",gap:9}}>
              <input value={chatIn} onChange={e=>setChatIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendChat()} placeholder="Ask me anything about gifts..." style={{flex:1,background:P.card,border:`1px solid ${P.border}`,borderRadius:11,padding:"13px 16px",fontSize:14,transition:"border-color .2s"}} onFocus={e=>e.target.style.borderColor=P.teal} onBlur={e=>e.target.style.borderColor=P.border}/>
              <button onClick={sendChat} disabled={chatLoading||!chatIn.trim()} style={{background:chatIn.trim()&&!chatLoading?`linear-gradient(135deg,${P.indigo},${P.teal})`:P.border,color:"#fff",border:"none",borderRadius:11,padding:"0 22px",fontSize:17,cursor:chatIn.trim()?"pointer":"default"}}>→</button>
            </div>
          </div>
        )}
        {tab==="stars" && <StarsTab stars={stars} history={starHistory} onRedeem={redeemReward}/>}
      </div>
    </div>
  );
}
