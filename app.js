  ...
  return html`<div style=${{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
    ${gifts.map((g,i) => html`
      <div key=${i} style=${{background:P.bg,border:`1px solid ${P.border}`,borderRadius:14,padding:"12px 14px",display:"flex",gap:12,alignItems:"center"}}>
        ...
        <button onClick=${()=>window.open(isHotel(g.name)?getHotel(g.name):isExp(g.name)?getExp(g.name):getUrl(g.name),"_blank")}
          ...
          ${isHotel(g.name)?"Book 🏨":isExp(g.name)?"Book →":"Buy →"}
        </button>
      </div>`)}
  </div>`;
}
...
FLOW:
1. First message: Ask WHO and OCCASION. Then list these questions each on a separate line:

• What's their personality like? (romantic, adventurous, practical...)
• Rough budget in mind?
• Experience or something they can keep?
• Any hobbies or things they're obsessed with?

2. After the user replies with ANY info: immediately give 5 gift recommendations, then ask ONE fun follow-up like "Want me to go more adventurous?" or "Shall I find something more sentimental?"

3. After EVERY follow-up: ALWAYS give 5 NEW refined recommendations + another follow-up. Never stop recommending after a follow-up.

Always use this format — one short punchy sentence first, then:
...
Mix variety: experiences, physical gifts, personalised options. Never use ** asterisks. Keep messages short and warm.`;
