...
function SmartBuyButton({name, city="Madrid"}) {
  ...
  const isHotel = /\b(hotel|stay|getaway|retreat|weekend away|resort|villa|cabin|cottage|airbnb)\b/i.test(name);
  const isNight = !isPhysical && /\b(club|nightclub|bar entry|bar crawl|party|hen|stag|vip table|vip entry|disco|rave|gig|live music|karaoke|night out|bottomless brunch|fabrik|kapital|joy eslava)\b/i.test(name) && !/\b(class|masterclass|workshop|lesson|tasting|course)\b/i.test(name);
  const isExp = !isPhysical && /\b(tour|class|workshop|tasting|lesson|experience|ride|cruise|show|concert|ticket|spa day|escape room|session|safari|hiking|climbing|sailing|diving|flight|paragliding|skydiving|bungee|rafting|kayaking|surfing|skiing|snowboarding|hot air balloon|zip line|snorkeling|voucher|adventure|walk|trekking|picnic|sunrise|sunset hike)\b/i.test(name);
  let url, label;
  if(isNight) {
    let found = null;
    for(const [k,v] of Object.entries(VENUES)) { if(lower.includes(k)) { found=v; break; } }
    url = found || `https://www.google.com/search?q=${encodeURIComponent(name+" Madrid")}`;
    label = "🎉 Book";
  } else if(isHotel) {
    url = `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(name)}&aid=7891172`;
    label = "🏨 Book";
  } else if(isExp) {
    url = `https://www.getyourguide.com/s/?q=${encodeURIComponent(name.split(/entry|access|ticket/i)[0].trim()+" "+city)}&partner_id=YHVA20C`;
    label = "Book →";
  } else {
  ...
  const handleClick = () => {
    track("gift_click", {gift_name: name, category, platform: isNight?"google":isHotel?"booking":isExp?"getyourguide":"amazon", city: c});
...
function MainApp({session, profile, setProfile}) {
  ...
  const TABS = [{id:"home",icon:"🏠",label:"Home"},{id:"search",icon:"🔍",label:"Search"},{id:"groups",icon:"👥",label:"Groups"},{id:"concierge",icon:"💬",label:"Concierge"},{id:"profile",icon:"👤",label:"Profile"}];
  ...
  return html`
    <div style=${{background:P.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:74}}>
      ...
      <div style=${{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:P.card,borderTop:`1px solid ${P.border}`,display:"flex",zIndex:100}}>
        ${TABS.map(t => html`
          <button key=${t.id} onClick=${()=>goTab(t.id)} style=${{flex:1,padding:"10px 0",background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
