...
function ConciergeTab({profile}) {
  ...
  const SYSTEM = `You are Giftmate, a warm and creative AI gift concierge. The user is ${profile.display_name}, based in ${city}, with interests: ${(profile.interests||[]).join(", ")||"various things"}. Always reply in this EXACT format — first a short warm intro sentence, then a JSON block inside <gifts> tags with an array of 3 gift objects: <gifts>[{"name":"...","description":"under 10 words","price":50,"emoji":"🎁","reason":"why perfect"}]</gifts>. Keep the intro to 1 sentence max. No markdown asterisks outside the gifts tag.`;
  ...
  // Auto-send greeting on load
  useEffect(() => { send("Give me 3 great gift ideas that blend perfectly with my interests and city."); }, []);
