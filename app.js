...
const THEMES = {
  midnight: { name:"Midnight", icon:"🌙", bg:"#0A0A18", card:"#12122A", border:"#ffffff18", text:"#F0F0FF", muted:"#9090B0", faint:"#50507080", gold:"#F4A438", goldL:"#FFD580", goldD:"#D4841A", teal:"#14B8A6", green:"#22C55E", red:"#EF4444" },
  rose:     { name:"Rose",     icon:"🌸", bg:"#150A10", card:"#221018", border:"#FF6B9D33", text:"#FFE8F2", muted:"#C07898", faint:"#80405880", gold:"#FF6B9D", goldL:"#FFB3D1", goldD:"#CC3D77", teal:"#FF8FAB", green:"#22C55E", red:"#EF4444" },
  lavender: { name:"Lavender", icon:"💜", bg:"#0E0A18", card:"#1A1228", border:"#C084FC33", text:"#F0E8FF", muted:"#A080C0", faint:"#70508080", gold:"#C084FC", goldL:"#DDB6FF", goldD:"#9333EA", teal:"#A78BFA", green:"#22C55E", red:"#EF4444" },
  blush:    { name:"Blush",    icon:"🎀", bg:"#180C10", card:"#281420", border:"#FB7EA833", text:"#FFEEF5", muted:"#C08898", faint:"#80506080", gold:"#FB7EA8", goldL:"#FDB8CC", goldD:"#E0507A", teal:"#F9A8C9", green:"#22C55E", red:"#EF4444" },
  ocean:    { name:"Ocean",    icon:"🌊", bg:"#08121E", card:"#0F1E30", border:"#38BDF833", text:"#E0F4FF", muted:"#6090B0", faint:"#40607080", gold:"#38BDF8", goldL:"#7DD3FC", goldD:"#0284C7", teal:"#22D3EE", green:"#22C55E", red:"#EF4444" },
  sage:     { name:"Sage",     icon:"🌿", bg:"#081408", card:"#102010", border:"#4ADE8033", text:"#E8F5E8", muted:"#70A070", faint:"#40604080", gold:"#4ADE80", goldL:"#86EFAC", goldD:"#16A34A", teal:"#34D399", green:"#4ADE80", red:"#EF4444" },
};
let _theme = "midnight";
...
function FriendProfile({friend, myProfile, following, onToggleFollow, onBack}) {
  ...
  const addBirthdayToCalendar = async () => {
    ...
    // Calculate next upcoming birthday (current or next year)
...
// ── MY PROFILE ──
function MyProfile({profile, setProfile, friendsOccasions=[], onLangChange, onThemeChange}) {
  const [occasions,setOccasions] = useState([]);
  const [wishlist,setWishlist] = useState([]);
  const [giftsReceived,setGiftsReceived] = useState([]);
  const [section,setSection] = useState("occasions");
  ...
  useEffect(() => {
    sb.from("follows").select("following_id").eq("follower_id", profile.id)
      .then(({data}) => setMyFollowing((data||[]).map(f=>f.following_id)));
  }, []);
  // Wishlist lists
  ...
  useEffect(() => {
    Promise.all([
      ...
      setOccasions(o.data||[]); setWishlist(w.data||[]); setGiftsReceived(g.data||[]);
  ...
  return html`<div>
    ${followModal && html`<${FollowListModal} userId=${profile.id} mode=${followModal} onClose=${()=>setFollowModal(null)} onViewProfile=${u=>{ setFollowModal(null); setViewingUser(u); }} myProfile=${profile} following=${myFollowing} onToggleFollow=${async id=>{ const isF=myFollowing.includes(id); if(isF){await sb.from("follows").delete().eq("follower_id",profile.id).eq("following_id",id); setMyFollowing(p=>p.filter(x=>x!==id));} else {await sb.from("follows").insert({follower_id:profile.id,following_id:id}); setMyFollowing(p=>[...p,id]);} }}/>`}
    ...
    const channel = sb.channel("inbox-"+profile.id)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"gift_messages",filter:`receiver_id=eq.${profile.id}`}, payload => {
        ...
        if(Notification.permission==="granted") {
          new Notification("🎁 You received a gift!", {body:`${payload.new.gift_name}`, icon:"/icon-192.png"});
        }
      }).subscribe();
    return () => { track("session_end", {}); sb.removeChannel(channel); };
  }, []);

  const goTab = t => { trackTabView(t); setTab(t); if(t==="profile") loadUnread(); };
  ...
  return html`
    <div style=${{background:P.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:74}}>
      ...
      <div style=${{padding:"16px 16px 0"}}>
        ...
        ${tab==="profile" && html`<${MyProfile} profile=${profile} setProfile=${setProfile} friendsOccasions=${feed.flatMap(f=>f.occasions)} onLangChange=${onLangChange} onThemeChange=${onThemeChange}/>`}
