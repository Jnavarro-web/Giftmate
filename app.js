...
function GiftCards({gifts, city}) {
  ...
  const getNight = name => `https://www.google.com/search?q=${encodeURIComponent(name+" "+city+" book tickets reservations")}`;
  ...
  const isNight = name => /\b(club|nightclub|bar|cocktail|party|hen|stag|birthday night|vip|rooftop|lounge|disco|rave|gig|live music|festival|concert night|karaoke|night out|drinks|bottomless brunch|entry|fabrik|fabrika)\b/i.test(name);
