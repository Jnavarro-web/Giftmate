...
function ConciergeTab({profile}) {
  const [messages, setMessages] = useState([{role:"assistant",content:`Hi ${profile.display_name}! 🎁 I'm your Giftmate concierge. Tell me about someone you want to gift and I'll find the perfect present. Who are you shopping for?`}]);
  ...
  const QUICK = ["Gift for my mum's birthday 🎂","Anniversary under €100 💕","Tech gift for a friend 💻","Surprise experience in Madrid ✈️"];
  ...
  const send = async text => {
    ...
    setLoading(true);
    try {
      const systemPrompt = `You are Giftmate, a warm and creative AI gift concierge. The user is ${profile.display_name}, based in ${profile.city||"Madrid"}, with interests: ${(profile.interests||[]).join(", ")||"various"}. Suggest thoughtful, specific gift ideas with prices. Keep responses concise and friendly. Always suggest 2-3 concrete options with approximate prices in euros.`;
      ...
      const data = await res.json();
      const reply = data.content?.[0]?.text || "Sorry, I couldn't think of anything right now — try again!";
      setMessages(p => [...p, {role:"assistant", content:reply}]);
    } catch(e) {
      setMessages(p => [...p, {role:"assistant", content:"Hmm, something went wrong. Try again in a moment! 🎁"}]);
    }
    setLoading(false);
  };
  ...
  return html`<div style=${{display:"flex",flexDirection:"column",height:"calc(100vh - 130px)"}}>
    <div style=${{marginBottom:12}}>
      <div style=${{fontWeight:800,fontSize:18,color:P.text,marginBottom:4}}>AI Concierge 💬</div>
    ...
    <div style=${{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:12,paddingBottom:12}}>
      ${messages.map((m,i) => html`
        <div key=${i} style=${{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",gap:8,alignItems:"flex-end"}}>
          ...
          <div style=${{maxWidth:"78%",background:m.role==="user"?`linear-gradient(135deg,${P.goldD},${P.gold})`:P.card,color:m.role==="user"?"#000":P.text,borderRadius:m.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px",padding:"11px 15px",fontSize:14,lineHeight:1.5,border:m.role==="assistant"?`1px solid ${P.border}`:"none",whiteSpace:"pre-wrap"}}>
            ${m.content}
          </div>
        </div>`)}
      ${loading && html`<div style=${{display:"flex",gap:8,alignItems:"flex-end"}}>
        ...
        <div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:"18px 18px 18px 4px",padding:"11px 15px",color:P.muted,fontSize:14}}>Thinking… ✨</div>
      </div>`}
      <div ref=${bottomRef}/>
    </div>
