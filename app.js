...
function EditProfileModal({profile, onSave, onClose}) {
  ...
  const save = async () => {
    if(!name.trim()) return;
    setLoading(true);
    const updates = {display_name:name, username:username.toLowerCase().replace(/[^a-z0-9_]/g,""), emoji, interests, avatar_url:avatarUrl||null};
  ...
  return html`
    <div style=${{position:"fixed",inset:0,background:"#000b",zIndex:2000,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick=${onClose}>
      <div style=${{background:P.card,borderRadius:"20px 20px 0 0",padding:24,width:"100%",maxWidth:480,maxHeight:"85vh",overflowY:"auto"}} onClick=${e=>e.stopPropagation()}>
        <div style=${{fontWeight:800,fontSize:18,color:P.text,marginBottom:20,textAlign:"center"}}>Edit Profile ✏️</div>
        ...
        <div style=${{textAlign:"center",marginBottom:20}}>
          ...
          ${avatarUrl && html`<div><button onClick=${()=>setAvatarUrl("")} style=${{background:"none",border:"none",color:P.muted,fontSize:12,cursor:"pointer",marginTop:6}}>Remove photo</button></div>`}
        </div>

        <div style=${{marginBottom:10}}>
          <div style=${{fontSize:11,color:P.muted,fontWeight:700,marginBottom:5}}>DISPLAY NAME</div>
          <${Inp} value=${name} onChange=${setName} placeholder="Your name"/>
        </div>
        <div style=${{marginBottom:14}}>
          <div style=${{fontSize:11,color:P.muted,fontWeight:700,marginBottom:5}}>USERNAME</div>
          <${Inp} value=${username} onChange=${v=>setUsername(v.toLowerCase().replace(/[^a-z0-9_]/g,""))} placeholder="username"/>
        </div>

        <div style=${{marginBottom:14}}>
          <div style=${{fontSize:11,color:P.muted,fontWeight:700,marginBottom:8}}>EMOJI AVATAR</div>
...
function SmartBuyButton({name, city="Madrid"}) {
  ...
  const isDayTrip = /\b(day trip|day out|visit to|trip to|excursion|guided tour|walking tour|train to|train ride|steam train|vintage train|route|itinerary)\b/i.test(name);

  const isPhysical = !LANDMARKS.test(name) && /\b(kit|set|book|journal|bag|box|bottle|device|gadget|tool|equipment|card|basket|hamper|map|print|poster|canvas|frame|photo|album|personaliz|customiz|engraved|bracelet|necklace|watch|wallet|perfume|scent|candle|seeds?)\b/i.test(name);
  const isExpFirst = LANDMARKS.test(name) || isDayTrip || /\b(hot springs|thermal baths?|spa day|wellness day|masterclass|cooking class|wine tasting|cocktail class|tour|tasting|escape room|safari|paragliding|skydiving|bungee|rafting|kayaking|surfing|skiing|snowboarding|hot air balloon|zip line|snorkeling)\b/i.test(name);
  const isHotel = !isExpFirst && (/\b(hotel|stay|getaway|retreat|weekend away|weekend in|city weekend|city break|glamping|resort|villa|cabin|cottage|airbnb|overnight|bed and breakfast|b&b)\b/i.test(name) || /\bweekend\b/i.test(name) && !/\b(class|masterclass|workshop|lesson|tasting|course|night out|bar|club)\b/i.test(name));
  const isNight = !isPhysical && !isHotel && !isExpFirst && /\b(club|nightclub|bar entry|bar crawl|party|hen|stag|vip table|vip entry|disco|rave|gig|live music|karaoke|night out|bottomless brunch|fabrik|kapital|joy eslava)\b/i.test(name) && !/\b(class|masterclass|workshop|lesson|tasting|course)\b/i.test(name);
  const isExp = !isPhysical && !isHotel && (isExpFirst || /\b(class|workshop|lesson|experience|ride|cruise|show|concert|ticket|session|hiking|climbing|sailing|diving|flight|voucher|adventure|walk|trekking|picnic|sunrise|sunset hike)\b/i.test(name));

  // ── Smart query extractors ──
  const bookingQuery = n => {
    // Strip filler words, keep location + type keywords
    return n
      .replace(/\b(golden|luxury|romantic|perfect|amazing|ultimate|special|exclusive|beautiful|stunning|charming)\b/gi, "")
      .replace(/\b(weekend getaway|city weekend|city break|weekend in|weekend away|stay in|stay at|getaway to)\b/gi, "")
      .replace(/\b(a |the |an )\b/gi, "")
      .replace(/\s+/g, " ").trim();
  };

  const gygQuery = n => {
    // Remove ticket/entry/access noise, keep activity + location
    const cleaned = n
      .replace(/\b(entry|access|ticket|voucher|gift|experience|session|day)\b/gi, "")
      .replace(/\b(golden|luxury|romantic|perfect|amazing|ultimate|special|exclusive)\b/gi, "")
      .replace(/\b(a |the |an )\b/gi, "")
      .replace(/\s+/g, " ").trim();
    // Only append city if not already mentioned
...
function CalendarView({occasions, friendsOccasions, onExport}) {
  ...
  return html`<div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:16,padding:16,marginBottom:14}}>
    ...
    <div style=${{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:6}}>
      ${["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => html`<div key=${d} style=${{textAlign:"center",fontSize:10,color:P.muted,fontWeight:700,padding:"4px 0"}}>${d}</div>`)}
    </div>
      ...
      ${Array.from({length:daysInMonth},(_,i)=>i+1).map(d => {
        ...
        return html`<div key=${d} style=${{minHeight:44,borderRadius:8,background:cellBg,border:cellBorder,padding:"3px 2px",display:"flex",flexDirection:"column",alignItems:"center",cursor:occs.length>0?"pointer":"default"}}>
          ...
          ${occs.slice(0,2).map(o => html`<div key=${o.id} style=${{width:"100%",marginTop:1,borderRadius:3,background:o.color||(o.mine?P.gold:P.teal),padding:"1px 2px",overflow:"hidden"}}>
            <div style=${{fontSize:7,fontWeight:700,color:"#000",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",lineHeight:"10px"}}>${o.type}</div>
          </div>`)}
        </div>`;
      })}
    </div>
    <div style=${{display:"flex",gap:10,marginTop:12,fontSize:11,color:P.muted,justifyContent:"space-between",alignItems:"center"}}>
      <div style=${{display:"flex",gap:10}}>
        <span><span style=${{color:P.gold}}>●</span> Yours</span>
        <span><span style=${{color:P.teal}}>●</span> Friends</span>
      </div>
      <button onClick=${exportICS} style=${{background:`${P.gold}22`,border:`1px solid ${P.gold}44`,color:P.goldL,borderRadius:8,padding:"5px 10px",fontSize:11,fontWeight:700,cursor:"pointer"}}>📅 Add to Phone</button>
    </div>
  </div>`;
}
  ...
  const MsgCard = ({m, isSent}) => html`
    <div style=${{background:P.card,border:`1px solid ${m.status==="thanked"?(isSent?P.green:P.border):P.gold+"66"}`,borderRadius:14,padding:16,marginBottom:10}}>
      <div style=${{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
        ...
        ${m.status==="pending" && html`<div style=${{color:isSent?P.muted:P.gold,fontSize:isSent?11:8,fontWeight:700}}>${isSent?"⏳ Pending":html`<div style=${{width:8,height:8,borderRadius:"50%",background:P.gold}}/>`}</div>`}
        ${m.status==="thanked" && html`<div style=${{color:isSent?P.green:P.green,fontSize:12,fontWeight:700}}>${isSent?"❤️ Thanked!":"✓ Thanked"}</div>`}
      </div>
      ...
      ${!isSent && m.status==="pending" && html`<button onClick=${()=>thankYou(m)} style=${{width:"100%",background:`linear-gradient(135deg,${P.goldD},${P.gold})`,border:"none",color:"#000",borderRadius:10,padding:"10px 0",fontSize:13,fontWeight:700,cursor:"pointer",marginTop:10}}>Say Thank You ❤️</button>`}
    </div>`;
  ...
  return html`<div>
    <div style=${{display:"flex",background:P.bg,borderRadius:10,padding:3,marginBottom:14,gap:2}}>
      <button onClick=${()=>setTab("received")} style=${{flex:1,padding:"8px 4px",borderRadius:8,border:"none",background:tab==="received"?`linear-gradient(135deg,${P.goldD},${P.gold})`:"transparent",color:tab==="received"?"#000":P.muted,fontWeight:700,fontSize:12,cursor:"pointer"}}>
        📬 Received ${pendingCount>0?html`<span style=${{background:P.red,color:"#fff",borderRadius:99,padding:"1px 5px",fontSize:10,marginLeft:3}}>${pendingCount}</span>`:""}
      </button>
      <button onClick=${()=>setTab("sent")} style=${{flex:1,padding:"8px 4px",borderRadius:8,border:"none",background:tab==="sent"?`linear-gradient(135deg,${P.goldD},${P.gold})`:"transparent",color:tab==="sent"?"#000":P.muted,fontWeight:700,fontSize:12,cursor:"pointer"}}>📤 Sent</button>
    </div>
    ${tab==="received" && html`<div>
      ${inbox.length===0&&html`<div style=${{color:P.muted,textAlign:"center",padding:24,fontSize:14}}>No gift messages yet!</div>`}
      ${inbox.map(m=>html`<${MsgCard} key=${m.id} m=${m} isSent=${false}/>`)}
    </div>`}
    ${tab==="sent" && html`<div>
      ${sent.length===0&&html`<div style=${{color:P.muted,textAlign:"center",padding:24,fontSize:14}}>You haven't sent any gift messages yet!</div>`}
...
function FriendProfile({friend, myProfile, following, onToggleFollow, onBack}) {
  ...
  const getGiftIdeas = async occ => {
    ...
    try {
      const res = await fetch(API, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:MODEL,max_tokens:700,messages:[{role:"user",content:`Generate 5 personalised gift ideas for ${friend.display_name} for their ${occ.type} in ${city}. Interests: ${(friend.interests||[]).join(", ")||"unknown"}. Past gifts: ${giftsReceived.map(g=>g.gift_name).join(", ")||"none"}. Wishlist: ${wishlist.map(w=>w.name).join(", ")||"empty"}. IMPORTANT: Mix physical products AND experiences (tours, classes, workshops, activities in ${city}). Return ONLY a valid JSON array, nothing else: [{"name":"...","description":"under 10 words","price":50,"emoji":"🎁"}]`}]})});
  ...
  return html`<div>
    ...
    <div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:20,padding:20,marginBottom:14,textAlign:"center"}}>
      ...
      ${friend.birthday && html`<div style=${{fontSize:13,color:P.gold,marginBottom:6}}>🎂 ${fmtDate(friend.birthday)} · in ${daysUntil(friend.birthday)} days</div>`}
      ${friend.birthday && html`<button onClick=${addBirthdayToCalendar} style=${{background:`${P.gold}11`,border:`1px solid ${P.gold}33`,color:P.goldL,borderRadius:8,padding:"5px 12px",fontSize:11,fontWeight:700,cursor:"pointer",marginBottom:10}}>📅 Add birthday to my calendar</button>`}
...
// ── MY PROFILE ──
function MyProfile({profile, setProfile, friendsOccasions=[]}) {
  const [occasions,setOccasions] = useState([]);
  const [wishlist,setWishlist] = useState([]);
  const [giftsReceived,setGiftsReceived] = useState([]);
  const [section,setSection] = useState("occasions");
  ...
  const addWish = async () => {
    ...
    track("wishlist_add", {item_name: newWish.name, price: newWish.price||null});
  };
  ...
  return html`<div>
    ${showEdit && html`<${EditProfileModal} profile=${profile} onSave=${p=>{setProfile(p);}} onClose=${()=>setShowEdit(false)}/>`}
    <div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:20,padding:20,marginBottom:14,textAlign:"center"}}>
      ...
      <div style=${{display:"flex",gap:8,justifyContent:"center",marginTop:14}}>
        <button onClick=${()=>setShowEdit(true)} style=${{background:`${P.gold}22`,border:`1px solid ${P.gold}44`,color:P.goldL,borderRadius:8,padding:"7px 16px",fontSize:12,fontWeight:700,cursor:"pointer"}}>✏️ Edit Profile</button>
        <button onClick=${()=>sb.auth.signOut()} style=${{background:"none",border:`1px solid ${P.border}`,color:P.muted,borderRadius:8,padding:"7px 16px",fontSize:12,cursor:"pointer"}}>Log out</button>
      </div>
    </div>
    ...
    ${section==="occasions" && html`<div>
      ...
      ${addingOcc ? html`<div style=${{background:P.card,border:`1px solid ${P.gold}44`,borderRadius:14,padding:16,marginBottom:10}}>
        <select value=${OCCASIONS.includes(newOcc.type)?newOcc.type:"custom"} onChange=${e=>{if(e.target.value==="custom"){setNewOcc(o=>({...o,type:""}))}else{setNewOcc(o=>({...o,type:e.target.value}))}}} style=${{width:"100%",background:P.bg,border:`1px solid ${P.border}`,borderRadius:8,padding:"10px",color:P.text,fontSize:14,marginBottom:10,boxSizing:"border-box"}}>
          ${OCCASIONS.map(o => html`<option key=${o} value=${o}>${o}</option>`)}
          <option value="custom">✏️ Custom…</option>
        </select>
        ${!OCCASIONS.includes(newOcc.type) && html`<input value=${newOcc.type} onInput=${e=>setNewOcc(o=>({...o,type:e.target.value}))} placeholder="e.g. Work Anniversary, Pet's Birthday…" style=${{width:"100%",background:P.bg,border:`1px solid ${P.gold}55`,borderRadius:8,padding:"10px",color:P.text,fontSize:14,marginBottom:10,boxSizing:"border-box",outline:"none"}}/>`}
        <div style=${{marginBottom:10}}>
        ...
        <div style=${{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
          ...
          <label for="occ_pub" style=${{color:P.muted,fontSize:13,cursor:"pointer"}}>Make this occasion public (friends can see it)</label>
        </div>
...
function GroupsTab({profile, following, feed}) {
  ...
  return html`<div>
    <div style=${{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div>
        <div style=${{fontWeight:800,fontSize:18,color:P.text}}>Gift Groups 🎁</div>
        <div style=${{color:P.muted,fontSize:13}}>Plan & split gifts with friends</div>
      </div>
      <button onClick=${()=>setShowCreate(true)} style=${{background:`linear-gradient(135deg,${P.goldD},${P.gold})`,border:"none",color:"#000",borderRadius:10,padding:"9px 16px",fontWeight:700,fontSize:13,cursor:"pointer"}}>+ New Group</button>
    </div>
    ...
    ${!loading && groups.length===0 && html`<div style=${{textAlign:"center",padding:40}}>
      ...
      <div style=${{fontWeight:700,fontSize:16,color:P.text,marginBottom:8}}>No groups yet!</div>
      <div style=${{color:P.muted,fontSize:14,marginBottom:20}}>Create a group to plan a mutual gift with friends and split the cost.</div>
      <button onClick=${()=>setShowCreate(true)} style=${{background:`linear-gradient(135deg,${P.goldD},${P.gold})`,border:"none",color:"#000",borderRadius:12,padding:"12px 24px",fontWeight:700,cursor:"pointer"}}>Create First Group 🎉</button>
    </div>`}
...
function GroupChat({group, profile, feed, following, onBack}) {
  ...
  return html`<div style=${{display:"flex",flexDirection:"column",height:"calc(100vh - 130px)"}}>
    ...
    <div style=${{display:"flex",gap:8,paddingTop:8,borderTop:`1px solid ${P.border}`}}>
      <input value=${input} onInput=${e=>setInput(e.target.value)} onKeyDown=${e=>e.key==="Enter"&&sendMsg()} placeholder="Message…" style=${{flex:1,background:P.card,border:`1px solid ${P.border}`,borderRadius:12,padding:"11px 14px",color:P.text,fontSize:14,outline:"none"}}/>
      ...
      <div style=${{background:P.card,borderRadius:"20px 20px 0 0",padding:24,width:"100%",maxWidth:480,maxHeight:"80vh",overflowY:"auto"}} onClick=${e=>e.stopPropagation()}>
        ...
        <div style=${{background:P.bg,borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:12,color:P.muted}}>
          ...
          <div style=${{display:"flex",gap:8,marginTop:8}}>
            <button onClick=${()=>setSplits(Object.fromEntries(members.map(m=>[m.user_id,even])))} style=${{background:`${P.gold}22`,border:`1px solid ${P.gold}44`,color:P.goldL,borderRadius:6,padding:"4px 10px",fontSize:11,cursor:"pointer",fontWeight:700}}>Split evenly (€${even} each)</button>
          </div>
        </div>
...
Mix: experiences, physical gifts, personalised, hotels, nightlife/events. BUT if the user is clearly talking about nightlife/clubs/going out, give ALL 5 as nightlife options — real specific venues, club nights, bar crawls, cocktail spots in ${city} by name. If they're talking about romantic experiences, go all-in on experiences. Match the energy of what they're asking for. No ** asterisks.`;
  const [messages, setMessages] = useState([{role:"assistant", content:`Hi ${name}! 🎁 I'm your personal gift concierge. Who are we finding a gift for today, and what's the occasion?`}]);
  const [rawMessages, setRawMessages] = useState([{role:"assistant", content:`Hi ${name}! 🎁 I'm your personal gift concierge. Who are we finding a gift for today, and what's the occasion?`}]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
...
function MainApp({session, profile, setProfile}) {
  ...
  const TABS = [{id:"home",icon:"🏠",label:"Home"},{id:"search",icon:"🔍",label:"Search"},{id:"groups",icon:"👥",label:"Groups"},{id:"concierge",icon:"💬",label:"GiftMind"},{id:"profile",icon:"👤",label:"Profile"},{id:"stars",icon:"⭐",label:"Stars"}];
  ...
  return html`
    <div style=${{background:P.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:74}}>
      ...
      <div style=${{padding:"16px 16px 0"}}>
        ${tab==="home" && (feedLoading
          ...
          : feed.length===0
            ? html`<div style=${{textAlign:"center",padding:40}}>
                ...
                <button onClick=${()=>goTab("search")} style=${{background:`linear-gradient(135deg,${P.goldD},${P.gold})`,color:"#000",border:"none",borderRadius:12,padding:"12px 28px",fontWeight:700,fontSize:14,cursor:"pointer"}}>Find Friends 🔍</button>
              </div>`
            : html`<div>
        ...
        ${tab==="profile" && html`<${MyProfile} profile=${profile} setProfile=${setProfile} friendsOccasions=${feed.flatMap(f=>f.occasions)}/>`}
