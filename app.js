...
function SmartBuyButton({name, city="Madrid"}) {
  ...
  const isPhysical = /\b(kit|set|book|journal|bag|box|bottle|device|gadget|tool|equipment|card|basket|hamper|map|print|poster|canvas|frame|photo|album|personaliz|customiz|engraved|bracelet|necklace|watch|wallet|perfume|scent|candle|seeds?)\b/i.test(name);
  const isExpFirst = /\b(hot springs|thermal baths?|spa day|wellness day|masterclass|cooking class|wine tasting|cocktail class|tour|tasting|escape room|safari|paragliding|skydiving|bungee|rafting|kayaking|surfing|skiing|snowboarding|hot air balloon|zip line|snorkeling)\b/i.test(name);
