...
function GiftCards({gifts, city}) {
  ...
  const getExp = name => `https://www.getyourguide.com/s/?q=${encodeURIComponent(name.split(/entry|access|ticket/i)[0].trim()+" "+(city||"Madrid"))}&partner_id=YHVA20C`;
  const getHotel = name => `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(name+" "+(city||""))}&aid=7891172`;
  const VENUES = {
    "kapital": "https://www.grupo-kapital.com",
    "fabrik": "https://www.fabricaclub.es",
    "fabrika": "https://www.fabricaclub.es",
    "sala el sol": "https://www.salaelsol.com",
    "el sol": "https://www.salaelsol.com",
    "teatro colon": "https://www.teatrocolonmadrid.com",
    "joy eslava": "https://www.joy-eslava.com",
    "joy": "https://www.joy-eslava.com",
    "charada": "https://www.charadamadrid.com",
    "mon": "https://www.monclubmadrid.com",
    "opium": "https://www.opium.es",
    "soho": "https://www.sohomadrid.es",
    "bash": "https://www.bashclub.es",
    "salmon guru": "https://www.salmonguru.es",
    "1862 dry bar": "https://www.1862drybar.com",
    "1862": "https://www.1862drybar.com",
    "la venencia": "https://www.lavenencia.com",
    "museo chicote": "https://www.museochicote.com",
    "chicote": "https://www.museochicote.com",
    "hemingway": "https://www.cafehemingwaymadrid.com",
    "the hat": "https://www.thehatmadrid.com",
    "azotea del circulo": "https://www.circulobellasartes.com",
    "el viajero": "https://www.elviajero.es",
  };
  const getVenueUrl = name => {
    const lower = name.toLowerCase();
    for(const [key, url] of Object.entries(VENUES)) {
      if(lower.includes(key)) return url;
    }
    return `https://www.google.com/search?q=${encodeURIComponent(name+" Madrid")}`;
  };
  const getNight = name => getVenueUrl(name);
  ...
  const isNight = name => /\b(club|nightclub|bar entry|bar crawl|party|hen|stag|birthday night|vip table|vip entry|rooftop bar|lounge|disco|rave|gig|live music|festival|concert night|karaoke|night out|bottomless brunch|fabrik|kapital|teatro colon|joy eslava)\b/i.test(name) && !/\b(class|masterclass|workshop|lesson|tasting|course|tutorial)\b/i.test(name);
  const isPhysical = name => /\b(kit|set|book|journal|bag|box|bottle|device|gadget|tool|equipment|card|basket|hamper|log|manual|map|print|poster|canvas|frame|photo|album|personaliz|customiz|engraved|bracelet|necklace|watch|wallet|perfume|scent|candle|plant|seeds?)\b/i.test(name);
  const isExp = name => !isPhysical(name) && /\b(tour|class|workshop|tasting|lesson|experience|ride|cruise|show|concert|ticket|spa day|escape room|session|safari|hiking|climbing|sailing|diving|flight|paragliding|skydiving|bungee|rafting|kayaking|surfing|skiing|snowboarding|hot air balloon|zip line|snorkeling|voucher|adventure|walk|trekking|picnic|sunrise|sunset hike)\b/i.test(name);
