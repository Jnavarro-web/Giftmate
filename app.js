  ...
  return html`<div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:16,padding:16,marginBottom:14}}>
    ...
    <div style=${{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
      ...
      ${Array.from({length:daysInMonth},(_,i)=>i+1).map(d => {
        ...
        const cellBg = isToday?`${P.gold}33`:hasMine?`${P.gold}22`:hasFriends?`${P.teal}22`:"transparent";
        const cellBorder = isToday?`1px solid ${P.gold}`:hasMine?`1px solid ${P.gold}55`:hasFriends?`1px solid ${P.teal}44`:"1px solid transparent";
        return html`<div key=${d} style=${{minHeight:44,borderRadius:8,background:cellBg,border:cellBorder,padding:"3px 2px",display:"flex",flexDirection:"column",alignItems:"center",cursor:occs.length>0?"pointer":"default"}}>
          ...
          ${occs.slice(0,2).map(o => html`<div key=${o.id} style=${{width:"100%",marginTop:1,borderRadius:3,background:o.mine?P.gold:P.teal,padding:"1px 2px",overflow:"hidden"}}>
