...
function EditProfileModal({profile, onSave, onClose, onLangChange, onThemeChange}) {
  ...
  const save = async () => {
    if(!name.trim()) return;
    setLoading(true);
    const updates = {display_name:name, username:username.toLowerCase().replace(/[^a-z0-9_]/g,""), emoji, interests, avatar_url:avatarUrl||null, city:city||null, country:country||null, language, theme:selectedTheme};
...
function MyProfile({profile, setProfile, friendsOccasions=[], onLangChange, onThemeChange}) {
  ...
  return html`<div>
    ${showEdit && html`<${EditProfileModal} profile=${profile} onSave=${p=>{setProfile(p);}} onLangChange=${onLangChange} onThemeChange=${onThemeChange} onClose=${()=>setShowEdit(false)}/>`}
    <div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:20,padding:20,marginBottom:14,textAlign:"center"}}>
...
function Giftmate() {
  ...
  return html`<${MainApp} key=${lang+theme} session=${session} profile=${profile} setProfile=${setProfile} onLangChange=${changeLang} onThemeChange=${changeTheme}/>`;
}
