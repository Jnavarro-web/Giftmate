...
function MyProfile({profile, setProfile, friendsOccasions=[], onLangChange, onThemeChange}) {
  ...
  return html`<div>
    ${followModal && html`<${FollowListModal} userId=${profile.id} mode=${followModal} onClose=${()=>setFollowModal(null)} onViewProfile=${u=>{ setFollowModal(null); setViewingUser(u); }} myProfile=${profile} following={[]} onToggleFollow=${()=>{}}/>`}
