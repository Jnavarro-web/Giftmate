  ...
  return html`<div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:16,padding:16,marginBottom:14}}>
    ...
    <div style=${{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
      ...
      ${Array.from({length:daysInMonth},(_,i)=>i+1).map(d => {
        ...
        return html`<div key=${d} style=${{minHeight:36,borderRadius:8,background:isToday?`${P.gold}33`:occs.length>0?`${P.teal}22`:"transparent",border:isToday?`1px solid ${P.gold}`:occs.length>0?`1px solid ${P.teal}44`:"1px solid transparent",padding:"3px",display:"flex",flexDirection:"column",alignItems:"center",cursor:occs.length>0?"pointer":"default"}} title=${occs.map(o=>o.type).join(", ")}>
          <span style=${{fontSize:11,color:isToday?P.gold:P.text,fontWeight:isToday?700:400}}>${d}</span>
          ${occs.slice(0,2).map(o => html`<div key=${o.id} style=${{width:4,height:4,borderRadius:"50%",background:o.mine?P.gold:P.teal,marginTop:1}}/>`)}
        </div>`;
      })}
    </div>
...
function ConciergeTab({profile}) {
  ...
  const SYSTEM = `You are Giftmate, the wittiest, most thoughtful AI gift concierge in the world. The user is ${profile.display_name}, based in ${city}, with interests: ${(profile.interests||[]).join(", ")||"various things"}.
