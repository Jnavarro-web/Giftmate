...
function GiftCards({gifts, city}) {
  ...
  const isExp = name => /\b(tour|class|workshop|tasting|lesson|experience|ride|cruise|show|concert|ticket|retreat|spa day|escape room)\b/i.test(name) && !/\b(kit|set|book|journal|bag|box|bottle|device|gadget|tool|gear|equipment|mat|board|card)\b/i.test(name);
