...
const THEMES = {
  // Midnight: classic dark gold — warm amber on deep navy
  ...
  // Rose: deep wine bg + hot pink + gold accent highlights
  rose:     { name:"Rose",     icon:"🌸", bg:"#130810", card:"#1E0E18", border:"#FF6B9D28", text:"#FFE8F4", muted:"#B06888", faint:"#70405080", gold:"#FF6B9D", goldL:"#FFAECF", goldD:"#C4316A", teal:"#F4A438", green:"#4ADE80", red:"#FF4D6D" },
  // Lavender: dark purple + violet + warm gold pop
  ...
  // Blush: warm rose-gold duotone — mauve bg + rose-gold accent
  blush:    { name:"Blush",    icon:"🎀", bg:"#160B10", card:"#241220", border:"#E8927828", text:"#FFEDF5", muted:"#B08090", faint:"#7850608f", gold:"#E89278", goldL:"#F5B8A8", goldD:"#C05A40", teal:"#FF6B9D", green:"#4ADE80", red:"#EF4444" },
  // Ocean: deep navy + sky blue + coral contrast
  ocean:    { name:"Ocean",    icon:"🌊", bg:"#060F1C", card:"#0D1C30", border:"#38BDF828", text:"#E0F4FF", muted:"#5888A8", faint:"#38587080", gold:"#38BDF8", goldL:"#7DD3FC", goldD:"#0272B0", teal:"#F4A438", green:"#4ADE80", red:"#F87171" },
  // Sage: forest night + mint + amber warmth
  sage:     { name:"Sage",     icon:"🌿", bg:"#06100A", card:"#0E1E12", border:"#4ADE8028", text:"#EAFAF0", muted:"#60986A", faint:"#40604880", gold:"#4ADE80", goldL:"#86EFAC", goldD:"#14A040", teal:"#F4A438", green:"#4ADE80", red:"#EF4444" },
};
let _theme = "midnight";
  ...
  return html`<div>
    ...
    <div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:20,padding:20,marginBottom:14,textAlign:"center"}}>
      ...
      <div style=${{display:"flex",gap:8,justifyContent:"center"}}>
        <button onClick=${()=>onToggleFollow(friend.id,friend)} style=${{background:isF?P.border:pendingRequests.includes(friend.id)?`${P.gold}44`:`linear-gradient(135deg,${P.goldD},${P.gold})`,border:pendingRequests.includes(friend.id)?`1px solid ${P.gold}`:"none",color:isF?P.muted:pendingRequests.includes(friend.id)?P.goldL:"#000",borderRadius:10,padding:"10px 20px",fontWeight:700,fontSize:14,cursor:"pointer"}}>${isF?t("following"):pendingRequests.includes(friend.id)?"Requested ⏳":t("follow")}</button>
...
function MyProfile({profile, setProfile, friendsOccasions=[], onLangChange, onThemeChange, following=[], onToggleFollow}) {
  ...
  const acceptReq = async req => {
    await sb.from("follow_requests").update({status:"accepted"}).eq("id",req.id);
    await sb.from("follows").insert({follower_id:req.requester_id, following_id:profile.id});
...
function MainApp({session, profile, setProfile, onLangChange, onThemeChange}) {
  ...
  useEffect(() => {
    ...
    return () => { track("session_end", {}); sb.removeChannel(channel); sb.removeChannel(groupChannel); sb.removeChannel(memberChannel); sb.removeChannel(reqChannel); };
  }, []);
  ...
  const loadFollowing = async () => {
    ...
    const {data:reqs} = await sb.from("follow_requests").select("target_id").eq("requester_id",profile.id).eq("status","pending");
  ...
  const toggleFollow = async (targetId, targetProfile) => {
    ...
    if(isF) {
      ...
      // Send follow request
      await sb.from("follow_requests").upsert({requester_id:profile.id, target_id:targetId, status:"pending"});
          ...
          ${searchResults.map(u => html`
            <div key=${u.id} style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:14,padding:"14px 16px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style=${{display:"flex",flexDirection:"column",gap:6}}>
                ...
                <button onClick=${()=>toggleFollow(u.id,u)} style=${{background:following.includes(u.id)?P.border:`linear-gradient(135deg,${P.goldD},${P.gold})`,border:"none",color:following.includes(u.id)?P.muted:"#000",borderRadius:8,padding:"5px 10px",fontSize:11,fontWeight:700,cursor:"pointer"}}>${following.includes(u.id)?t("following"):t("follow")}</button>
              </div>
            </div>`)}
