...
function SmartBuyButton({name, city="Madrid"}) {
  ...
  return html`<button onClick=${()=>window.open(url,"_blank")} style=${{background:`linear-gradient(135deg,${P.goldD},${P.gold})`,border:"none",color:"#000",borderRadius:8,padding:"6px 10px",fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>${label}</button>`;
}
  ...
  return html`<div style=${{background:P.card,border:`1px solid ${P.border}`,borderRadius:16,padding:16,marginBottom:14}}>
    ...
    <div style=${{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
      ...
      ${Array.from({length:daysInMonth},(_,i)=>i+1).map(d => {
        ...
        return html`<div key=${d} style=${{minHeight:44,borderRadius:8,background:isToday?`${P.gold}33`:occs.length>0?`${P.teal}22`:"transparent",border:isToday?`1px solid ${P.gold}`:occs.length>0?`1px solid ${P.teal}44`:"1px solid transparent",padding:"3px 2px",display:"flex",flexDirection:"column",alignItems:"center",cursor:occs.length>0?"pointer":"default"}}>
