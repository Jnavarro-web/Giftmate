...
function GiftCards({gifts, city}) {
  ...
  const isExp = name => /\b(tour|class|workshop|tasting|lesson|experience|ride|cruise|show|concert|ticket|retreat|spa day|escape room|session|safari|hiking|climbing|sailing|diving|cooking class|yoga class|dance class|flight|paragliding|skydiving|bungee|rafting|kayaking|surfing|skiing|snowboarding|hot air balloon|zip line|snorkeling)\b/i.test(name) && !/\b(kit|set|book|journal|bag|box|bottle|device|gadget|tool|gear|equipment|mat|board|card|basket|hamper|log)\b/i.test(name);
...
Keep it snappy — ask ONE quick question to learn a bit more, then immediately give gift recommendations. Never ask more than one follow-up question before recommending.

Once you have any info at all, give recommendations in this EXACT format — one short warm sentence, then gift cards:
